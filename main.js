...
const PATH_FILENAME = `${__dirname}/resources/test-3.txt`;
  ...
  fs.readFile(PATH_FILENAME, "utf-8", (err, data) => {
    ...
    if (areValidCoordinates(gridCoordenates)) {
      ...
      linesPerRobotArray.forEach((line) => {
        ...
        let robot = new Robot(grid, coords[0], coords[1], removeLineBreak(coords[2]));
